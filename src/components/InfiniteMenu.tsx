...
import React, { FC, useRef, useState, useEffect, MutableRefObject, useMemo } from 'react';
...
class InfiniteGridMenu {
  ...
  private render(): void {
    ...
    gl.clearColor(0.05098, 0.05098, 0.05098, 1.0); // Corresponds to brand-dark #0D0D0D
  ...
  const defaultItems: MenuItem[] = [
    {
        ...
        link: '#',
        title: 'Loading...',
        description: 'Please add items to the portfolio.'
    }
  ];

  const displayItems = items.length ? items : defaultItems;

  const itemsKey = useMemo(() => JSON.stringify(displayItems.map(i => i.image)), [displayItems]);

  useEffect(() => {
    const canvas = canvasRef.current;
    let sketch: InfiniteGridMenu | null = null;

    // Set initial state after a short delay to allow canvas to render
    const initialTimer = setTimeout(() => {
        setIsMoving(false);
    }, 1500); // Should match canvas fade-in
        ...
        <div className="info-content">
           ...
           {activeItem && (
            <>
                ...
                <div onClick={handleButtonClick} className="action-button" aria-label="View project">
